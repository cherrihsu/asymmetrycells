# -*- coding: utf-8 -*-
"""
This is a program that simulates the production and degradation of a diffusible component (U) and its membrane association (M) at the both ends of the cell.  In particular we scan for the steady-state concentration for U and the max. allowed steady-state value for M
"""
import numpy as np
...
if __name__ == "__main__" :  
    ...
    name = "scan_ss_data.npy"
    pool_n = 2#32 # number of processes in parallel
    para_size = 50#50 # number of grids to scan
    uss = np.logspace(0, 3, para_size)
    mss = np.logspace(2, 5, para_size)
    para = np.meshgrid(uss, mss)
